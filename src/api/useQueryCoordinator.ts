...
type QueryRunnerFunc = (query: string, index: number, options: SearchOptions) => Promise<string>

type QueryStore<T> = MutableRefObject<{ [query: string]: Array<T> }>

export interface QueryExecutor extends QueryHandler {
    execute: (QueryRunnerFunc) => (queries: string[], options: SearchOptions) => void
...
export const useQueryCoordinator = (): QueryExecutor => {
    ...
    const execute = useCallback((runQuery: QueryRunnerFunc) => (queries: string[], options: SearchOptions) => {
        setStatus('loading')
        const filteredQueries = queries.filter(q => q.trim().length > 0)
        report.reset(filteredQueries.length)
        rawData.current = {}
        Promise.allSettled(filteredQueries.map((q, i) => runQuery(q, i, options)))
            .then(promiseResults => {

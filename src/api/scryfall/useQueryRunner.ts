...
export const useScryfallQueryRunner = ({
    ...
    const {
        status, result, report, cache, rawData, execute
    } = useQueryCoordinator()

    const runQuery = (query: string, index: number, options: SearchOptions) => new Promise((resolve, reject) => {
        ...
        if (cache.current[_cacheKey] === undefined) {
            ...
            Scry.Cards.search(preparedQuery, options).on("data", data => {
                ...
                cache.current[_cacheKey].push({ data, weight, matchedQueries: [query] })
                report.addCardCount()
            }).on("done", () => {
                report.addComplete()
                resolve(query)

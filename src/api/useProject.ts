...
export const useProject = (): Project => {
    const [savedCards, setSavedCards] = useLocalStorage<string[]>("saved-cards", [])
    const addCard = useCallback(
        (next) => setSavedCards(prev => [...prev.filter(it => it.length > 0), next]),
        [setSavedCards]
    )

    const [ignoredIds, setIgnoredIds] = useLocalStorage<string[]>("ignore-list",[])
    const addIgnoredId = useCallback(
        (next) => setIgnoredIds(prev => [...prev.filter(it => it.length > 0), next]),
        [setIgnoredIds]
    )
    ...
    return {
        ...
        ignoredIds/*, setIgnoredIds*/, addIgnoredId,
        reset
    }
}
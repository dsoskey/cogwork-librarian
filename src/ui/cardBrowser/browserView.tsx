...
export const BrowserView = React.memo(({ addCard, result, status, source, report }: BrowserViewProps) => {
    ...
    const [visibleDetails, setVisibleDetails] = useLocalStorage("visible-details", ['weight', 'queries'])
    ...
    return <div className='results'>
        <div className="result-controls">
            {status === "loading" && <h2>running queries against {source}. please be patient...</h2>}

            <div className="topbar">
                <div>
                    {displayableCards.length > 0 && `${lowerBound} – ${Math.min(upperBound, displayableCards.length)} of ${displayableCards.length} cards. ignored ${ignoredCards.length} cards`}
                    {displayableCards.length === 0 && status !== "loading" && "0 cards found. We'll have more details on that soon :)"}
                    <div>
                        ...
                        {status !== 'loading' && <>
                            {report.start && report.end && `${source} query ran in ${(report.end - report.start) / 1000}s`}
                            ...
                            <div>
                                ...
                                {revealDetails && <div className="detail-controls">
                                    <div>
                                    ...
                                    <div>
                                        <input id="show-queries" type='checkbox'
                                               ...
                                               onChange={() => setVisibleDetails((prev) => {
                                                   const next = new Set(prev)
                                                   if (next.has(QUERIES)) {
                                                       next.delete(QUERIES)
                                                   } else {
                ...
                <div>
                    {displayableCards.length > 0 && <PageControl
                        page={page}
                        setPage={setPage}
                        pageSize={pageSize}
